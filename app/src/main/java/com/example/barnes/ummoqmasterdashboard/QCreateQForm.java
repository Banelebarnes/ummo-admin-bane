 ...
 */
public class QCreateQForm extends SingleInputFormActivity implements QMasterListener
{
   ...
   @Override
    ...
    public void registered(String string) {
        if (string.equals("SUCCESS")){
            Intent login_register = new Intent(this, AnimateToolbar.class);
    ...
    protected void onFormFinished(Bundle data)
    {
        ...
        String frame = "{start:'"+SeekBarStep.progress(data, DATA_TIME_FRAME_START)+"',end:'"+SeekBarStep.progress(data, DATA_TIME_FRAME_END)+"'}";
        qMaster.createQ(frame,TextStep.text(data, DATA_QUE_NAME),TextStep.text(data, DATA_QUE_TAG));
